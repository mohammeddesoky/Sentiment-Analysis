...
model_lr = joblib.load('logistic_reg_model.pkl')
...
with st.sidebar:
    ...
    st.write("----------------------")
    st.write("### Model Naive Bayes Info:")
    st.write("Sentiment Analysis Model")
    st.write("Train Accuracy: 99%")
    st.write("Test Accuracy: 94%")
    st.write("----------------------")
    st.write("### Model Logistic Regression Info:")
    st.write("Sentiment Analysis Model")
    st.write("Train Accuracy: 99%")
    st.write("Test Accuracy: 93%")
    st.write("----------------------")
...
if st.button("Analyze Sentiment"):
    if user_input:
        ...
        if choose_model == 'SVM':
            ...
            prediction = model_nb.predict(user_input_tfidf)
        elif choose_model == 'Logistic Regression':
            prediction = model_lr.predict(user_input_tfidf)
...
if uploaded_file:
    ...
    if set(expected_columns).issubset(data.columns):
        ...
        if choose_model == 'SVM':
            ...
            predictions = model_nb.predict(text_data_tfidf)
        elif choose_model == 'Logistic Regression':
            predictions = model_lr.predict(text_data_tfidf)
