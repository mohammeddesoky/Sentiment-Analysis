...
if st.button("Analyze Sentiment"):
    if user_input:
        ...
        # Make the prediction
        prediction_svm = model_svm.predict(user_input_tfidf)
        prediction_rf = model_rf.predict(user_input_tfidf)

        # Model SVM
        if choose_model == 'SVM':
            # Display sentiment with visualization
            if prediction_svm[0] == "Positive":
                st.success("Positive 😊")
            elif prediction_svm[0] == "Negative":
                st.error("Negative 😠")
            elif prediction_svm[0] == "Neutral":
                st.warning("Neutral 😐")
            else:
                st.warning("Irrelevant")
        # Model Random Forest
        else:
            if prediction_rf[0] == "Positive":
                st.success("Positive 😊")
            elif prediction_rf[0] == "Negative":
                st.error("Negative 😠")
            elif prediction_rf[0] == "Neutral":
                st.warning("Neutral 😐")
            else:
                st.warning("Irrelevant")
